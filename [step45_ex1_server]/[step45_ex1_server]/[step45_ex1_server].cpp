// [step45_ex1_server].cpp : 애플리케이션에 대한 진입점을 정의합니다.
//

#include "framework.h"
#include "[step45_ex1_server].h"
...
typedef struct {    // PACKET의 data 내용
    ...
    WCHAR fromID[BUFSIZE2]; // 유니코드

    unsigned short lenTo;
    WCHAR toID[BUFSIZE2];   // 유니코드

    unsigned short ix;  // GAME_END 일 때는 winner 저장
    unsigned short iy;
}GAME_DATA;
...
INT_PTR CALLBACK DlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_INITDIALOG:
    {
        ...
        addr.sin_port = 20000;
        int ret;
        ret = inet_pton(AF_INET, "loopback", &addr.sin_addr.s_addr);
    ...
    case WM_SERVER:
        ...
        case FD_READ:   // 패킷 분석
        {
            ...
            case GAME_END:
            {
                ...
                // toID의 소켓을 찾아 보냄
                it = clients.find(wstring(gameData.toID));
                if (it != clients.end()) // found
                    send(it->second, (char*)&packet, sizeof(PACKET), 0);
                break;
            }
            }   // end of switch
